...
<ParamField path="comment_render_mode" type="string" default="basic">
  How to render plan output in comments. Options: `basic`, `group_by_module`, `accumulate_plans`.

  With `accumulate_plans` the backend collects the plans of a run into a small number of comments — up
  to `reporting.max_plans_per_comment` plans each — instead of one comment per project. Requires
  `report_terraform_outputs` to stay enabled, and is supported on GitHub only.
</ParamField>
...
<ParamField path="reporting.max_plans_per_comment" type="integer" default="8">
  How many plans one comment holds under `comment_render_mode: accumulate_plans`. Must be at least 1,
  and is ignored by the other render modes.
</ParamField>
